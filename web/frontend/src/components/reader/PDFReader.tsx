...
export function PDFReader({ document: doc, onTextSelect }: PDFReaderProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Toolbar */}
      <div className="flex items-center gap-4 px-4 py-2 bg-[var(--warm-white)] border-b border-[var(--cool-gray)]">
        <div className="flex items-center gap-2">
          <button
            onClick={() => setPageNumber((p) => Math.max(1, p - 1))}
            disabled={pageNumber <= 1}
            className="px-2 py-1 text-xs bg-white border border-[var(--cool-gray)] rounded hover:bg-[var(--page-bg)] disabled:opacity-30"
          >
            ‹
          </button>
          <span className="text-xs tabular-nums text-[var(--secondary-text)]">
            <input
              ...
              className="w-10 text-center border border-[var(--cool-gray)] rounded text-xs py-0.5"
            />
        ...
        <div className="flex items-center gap-1 ml-auto">
          ...
          <span className="text-[10px] text-[var(--tertiary-text)] w-10 text-center">{Math.round(scale * 100)}%</span>
          <button onClick={() => setScale((s) => Math.min(2.5, s + 0.1))} className="px-2 py-1 text-xs bg-white border rounded">+</button>
        </div>
      </div>

      {/* PDF Canvas */}
      <div className="flex-1 overflow-auto bg-[var(--cool-gray)] flex justify-center p-4" onMouseUp={handleTextSelection}>
