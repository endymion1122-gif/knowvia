  ...
  if (!started) {
    return (
      <div className="p-8 max-w-2xl mx-auto">
        ...
        <button onClick={() => navigate("/pathways")} className="mt-4 text-xs text-[var(--brand-violet)] hover:underline">← 返回路径列表</button>
      </div>
    );
  }
    ...
    <div className="p-8 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => { setStarted(false); setItems([]); }}
          className="text-xs text-[var(--brand-violet)] hover:underline">← 重新选择</button>
      </div>
          ...
          <div key={item.card.id} className="bg-white p-4 rounded-xl border border-[var(--border-default)]">
            ...
            {!item.revealed ? (
              ...
              <div className="space-y-1">
                ...
                {item.userAnswer && (
                  <p className="text-[10px] text-[var(--text-tertiary)]">你的回答：{item.userAnswer}</p>
                )}
                <button onClick={() => {
      ...
      {items.every((i) => i.revealed) && (
        <div className="mt-6 bg-[var(--teal-100)] p-4 rounded-xl text-center">
          ...
          <div className="flex gap-2 justify-center mt-3">
            ...
            <button onClick={() => { setStarted(false); setItems([]); }}
              className="px-4 py-1.5 text-xs text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
              换一个路径
            </button>
          </div>
        </div>
